  ...
  if ('op' in op) {
    ...
    if (!source && this.type === types.defaultType && op.op.length > 1) {
      ...
      for (var i = 0; i < op.op.length; i++) {
        ...
        // this._setData(this.type.apply(this.data, componentOp.op)); // KF-MERGE-MARKER
    ...
    // this._setData(this.type.apply(this.data, op.op)); // KF-MERGE-MARKER
...
Doc.prototype._applyOp = function(op, undoOp, source, undoable, fixUp) {
  if (undoOp == null && (undoable || fixUp || op.needsUndoOp)) {
    if (this.type.applyAndInvert) {
      ...
      this._setData( result[0] );
      undoOp = { op: result[1] };
    } else {
      this._setData( this.type.apply(this.data, op.op) );
      if (this.type.invertWithDoc) // if available, use invertWithDoc
        undoOp = { op: this.type.invertWithDoc(op.op, this.data) };
      else
